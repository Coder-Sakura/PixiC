...
@app.route('/api/v2/get-info',methods=['POST'])
def get_info():
	"""
	查询指定pid的信息
	"""
	...
	# 指定数据表
	table = params.get('table',"bookmark")
...
@app.route('/api/v2/random',methods=['POST'])
def random_info():
	apier = get_apier()
	db = apier.db
	params = request.form
	num = params.get('num',1)				# 数量
	ex = params.get('extra',None)			# 指定tag
	limit = params.get('limit',None)		# 指定最小收藏数,如:'1000'
	illust_level = params.get('illust_level',None)	# 指定评分区间,如指定作品在R,SR之中
	is_r18 = params.get('is_r18',"True")	# 是否开启R-18,默认开启
	table = params.get('table',"bookmark")	# 指定数据表
	...
	for _ in random_pid_list:
		r = db.select_illust(_["pid"],table=table)
		if not r:
			continue
		item = r[0]
		del item["path"]
		item["reverse_url"] = db.pixiv_re_proxy(item)
		_urls = json.loads(item["urls"].replace("'",'"'))
		item["reverse_url_api_original"] = _urls["original"].replace("i.pximg.net","i.pixiv.re")
		item["reverse_url_api_regular"] = _urls["regular"].replace("i.pximg.net","i.pixiv.re")
		res.append(item)
...
@app.route('/api/v2/check_item',methods=['POST'])
def check_item():
	apier = get_apier()
	db = apier.db
	params = request.form
